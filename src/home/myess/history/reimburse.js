...
export default class Reimburse extends Component {
  ...
  _showReimburseMenu = (i) => {

    Alert.alert(
      'Reimburse Data',
      'Reimburse '+(parseInt(i)+1).toString(),
      [
        {text: 'View Detail', onPress: () => this.props.navigation.navigate("DetailReimburse",{reimburse:i})},
      ...
      listdetail.push(
        <Card  key={i} >
          <CardItem>
            ...
            <View style={styles.buttonRight}>
              <Icon onPress={() => this._showReimburseMenu(i)} style={{color:'grey'}} name="ios-more"/>
            </View>
            <View>
      ...
      <Container>
          ...
          <Left><Icon style={{color:'white', width:50}} name="arrow-back" onPress={() => this.props.navigation.goBack()}/></Left>
            <Body style={{justifyContent: 'center', alignItems: 'center', flex:100,marginLeft: 50, marginRight: 50,width:'100%',}}>
              <Title style={{color: "white",fontWeight: "300" }}>Reimburse History</Title>
            </Body>
            <Right/>
          </Header>
