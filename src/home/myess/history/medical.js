...
export default class Medical extends Component {
  ...
  _showMedicalMenu = (i) => {
    ...
    Alert.alert(
      'Medical Data',
      'Rawat Inap '+(parseInt(i)+1).toString(),
      [
        {text: 'View Detail', onPress: () => this.props.navigation.navigate("DetailMedical",{medical:i})},
      ...
      listdetail.push(
        <Card  key={i} >
          <CardItem>
            ...
            <View style={styles.buttonRight}>
              <Icon onPress={() => this._showMedicalMenu(i)} style={{color:'grey'}} name="ios-more"/>
            </View>
            <View>
      ...
      <Container>
          ...
          <Left><Icon style={{color:'white', width:50}} name="arrow-back" onPress={() => this.props.navigation.goBack()}/></Left>
            <Body style={{justifyContent: 'center', alignItems: 'center', flex:100,marginLeft: 50, marginRight: 50,width:'100%',}}>
              <Title style={{color: "white",fontWeight: "300" }}>Medical History</Title>
            </Body>
            <Right/>
          </Header>
