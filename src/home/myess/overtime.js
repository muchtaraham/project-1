...
export default class Overtime extends Component {
  ...
  render() {
    ...
    ShowOverTimeHideTextComponentView = (i) =>{
  	...
  	for(let i = 0; i < 2; i++){
        ...
        <Card  key={i} >
          <CardItem>
            <Body>
            <TouchableOpacity onPress={() => ShowOverTimeHideTextComponentView(i)} style={{width:'100%'}}>
            {this.state.status[i] ?
              <View style={styles.buttonRight}>
              <Icon onPress={() => ShowOverTimeHideTextComponentView(i)} style={{color:'grey'}} name="ios-arrow-down"/>
            </View>
            :
            <View style={styles.buttonRight}>
              <Icon onPress={() => ShowOverTimeHideTextComponentView(i)} style={{color:'grey'}} name="ios-more"/>
            </View>}
            <View>
